SELECT
    ...
    strategy_name,
    start_date,
    date_trunc('week', start_date) AS week_start -- добавляем дату начала недели
FROM collection_cases
WHERE start_date BETWEEN DATE '2026-01-01' AND DATE '2026-03-31';

---количество кейсов по неделе и стратегии(total_cases)---
...
---количество RPC кейсов за 7 дней(rpc_cases_7d)---

SELECT
    date_trunc('week', cc.start_date) AS week_start,
    cc.strategy_name,
    COUNT(DISTINCT cc.case_id) AS rpc_cases_7d
...
GROUP BY
    date_trunc('week', cc.start_date),
    cc.strategy_name
ORDER BY
	week_start,
	cc.strategy_name;

---Количество кейсов с обещанием(promise) за 7 дней(promise_cases_7d)---

SELECT
    date_trunc('week', cc.start_date) AS week_start,
    cc.strategy_name,
    COUNT(DISTINCT cc.case_id) AS promise_cases_7d
FROM collection_cases cc
JOIN contact_events ce
    ON ce.case_id = cc.case_id
...
---Первое обещание(promise) (<7 дней)---

WITH first_promises AS (
    SELECT
        cc.case_id,
...
SELECT
    ...
    date_trunc('week', cc.start_date) AS week_start,
    cc.strategy_name,
    fp.first_promise_dttm,
    ce.promise_to_pay_date,
    ce.promised_amount
FROM first_promises fp
JOIN collection_cases cc
    ON cc.case_id = fp.case_id
JOIN contact_events ce
    ON ce.case_id = fp.case_id
    AND ce.event_dttm = fp.first_promise_dttm
ORDER BY fp.case_id;

---Выполненные обещания (kept_promise_cases)---
WITH first_promises AS (-- 1.первое обещание по кейсу
    SELECT
        cc.case_id,
        MIN(ce.event_dttm) AS first_promise_dttm
    FROM collection_cases cc
...
FROM (
    SELECT
        ...
        SUM(p.payment_amount) AS paid_until_promise_date -- 2.Cуммируем платежи
...
WHERE t.paid_until_promise_date >= t.promised_amount -- 3. Сравниваем
GROUP BY
    t.week_start,
    t.strategy_name
ORDER BY
...
---Сумма сборов за 14 дней (recovery_amount_14d)---
SELECT
    date_trunc('week', cc.start_date) AS week_start,
    cc.strategy_name,
...
JOIN payments p
    ...
    AND p.payment_amount > 0
    AND p.payment_dttm::date BETWEEN cc.start_date AND cc.start_date + 13
WHERE cc.start_date BETWEEN DATE '2026-01-01' AND DATE '2026-03-31'
GROUP BY
    date_trunc('week', cc.start_date),
    cc.strategy_name
...
---Итоговая сборка всего этого добра---
WITH base_cases AS (
    SELECT
        cc.case_id,
        cc.account_id,
