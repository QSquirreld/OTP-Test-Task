...
/*
Сначала формирую базу кейсов за период январь–март 2026 года
и считаю по week_start и strategy_name отдельные метрики:
- общее число кейсов,
- число кейсов с RPC в первые 7 дней,
- число кейсов с обещанием в первые 7 дней,
- сумму сборов за 14 дней.
Для расчёта kept_promise_cases отдельно определяю первое обещание по кейсу через MIN(event_dttm),
затем суммирую положительные платежи по account_id до promise_to_pay_date включительно
и проверяю условие sum(payment_amount) >= promised_amount.
После этого соединяю все метрики по week_start и strategy_name
и считаю конверсионные показатели воронки.
*/